...
export function parseRetryAfterMs(value, nowMs) {
  ...
  if (Number.isNaN(at))
    return;
  if (new Date(at).toUTCString() !== trimmed)
    return;
...
export class ApiSession {
  ...
  lastRateLimitAt = 0;
  cap;
  constructor(baseInterval, transport, deps = {
    ...
    for (;; ) {
      ...
      if (result.kind === "unobservable-failure") {
        ...
        if (transportAttempt >= TRANSPORT_BACKOFF_MS.length)
          throw new TransportExhaustedError(url);
  ...
  onSuccess() {
    this.successStreak++;
    if (this.successStreak < DECAY_SUCCESS_STREAK)
      return;
    if (this.lastRateLimitAt !== 0 && this.deps.now() - this.lastRateLimitAt < DECAY_QUIET_MS)
      return;
